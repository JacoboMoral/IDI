...
void MyGLWidget::init_colFocus(){
    glm::vec3 colFocus(0.8, verd, 0.8);
    glUniform3fv(colFocusLoc,1,&colFocus[0]);
}
void MyGLWidget::initialize_Camera2 ()
{
    OBS2 = glm::vec3(0, 2.5, 0);
    FOV2 = M_PI/2.0;
    znear2 = 0.1;
    zfar2 = 10;
    VRP2 = glm::vec3(0,2.5,2.5);
}
...
void MyGLWidget::viewTransform ()
{
 ...
 if(!segonaCamera){
     ...
     View = glm::lookAt(OBS2, VRP2, glm::vec3(0, 1, 0));
  }
...
// Creació del Vertex Array Object del terra
  ...
  // Buffer de component difusa
  glGenBuffers(1, &VBO_TerraMatdiff);        projectTransform ();
  ...
  switch (event->key()) {
    case Qt::Key_O: { // canvia òptica entre perspectiva i axonomètrica
      ...
      case Qt::Key_R: { // canvia òptica entre perspectiva i axonomètrica
        rotar += 1;
        emit setRotar(int(rotar));
        break;
      }
     case Qt::Key_C: { // canvia òptica entre perspectiva i axonomètrica
        segonaCamera = !segonaCamera;
        viewTransform();
        projectTransform();
        break;
      }
    case Qt::Key_V: { // canvia òptica entre perspectiva i axonomètrica
        verd -= 0.1;
        init_colFocus();
        break;
      }
...
void MyGLWidget::rotarPatricios(int value){
    makeCurrent();
    rotar = value * M_PI/180;
    update();
}
