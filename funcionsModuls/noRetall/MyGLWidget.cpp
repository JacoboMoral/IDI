...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent)
{
  ...
  radiEsc = sqrt(5);
  angleY = angleX = 0.0;
  modelHeight = 2; //alcada del model, depen d’aquest
}
...
void MyGLWidget::initCamera(){
    viewTransform();
    //Calcul de parametres inicials de camera perspectiva
    initFOV = FOV = 2.0*asin(radiEsc/distancia);
    ra = 1.f;
    //calcul de parametres inicials de camera ortogonal
    right = top = radiEsc;
    left = bottom = -radiEsc;
    projectTransform();
}
...
void MyGLWidget::maxSize() {
    ...
    sceneMax = glm::vec3(2, 1, 2);
    sceneMin = glm::vec3(-2, -1, -2); //alçada min = -1, que es on es situa el terra
    radiEsc = glm::distance(sceneMax,sceneMin) / 2;
    distancia = radiEsc*2.f;
    
    for (int i = 0; i < 3; ++i){
        VRP[i] = (sceneMax[i] + sceneMin[i])/2.0;
    }
}
...
void MyGLWidget::resizeGL (int width, int height){
	ra = (float)width/(float)height; // ra(v) = raw

    //Recalculem el FOV per tal que no es retalli quan raw<1
    if (width< height) {
        FOV = 2 * atan(tan(initFOV /2.f)/ra); //perspec
        bottom = -radiEsc / ra; //ortho
        top = radiEsc / ra; //ortho
    }
    
    if (width > height) {
        left = -radiEsc * ra; //ortho
        right = radiEsc * ra; //ortho
    }
