...
void MyGLWidget::initializeGL ()
{
	...
	calcul_Contenidora();
    //Amb el valor d'escalat deduim el radi(escalat/2) i paràmetres ortho
    escalat = 2; //definit també al .hh per tal d'usarlo per al terra
...
void MyGLWidget::paintGL () 
{
    ...
    //El terra només es mostrara en perspective
...
{	
    float f_esc = escalat/(puntmax.y-puntmin.y);
    
    // Matriu de transformació del Model
	glm::mat4 transform (1.0f);
	transform = glm::scale(transform, glm::vec3(scale));
	transform = glm::rotate(transform, rotate_x, glm::vec3(1,0,0)); 
	transform = glm::rotate(transform, rotate_y, glm::vec3(0,1,0)); 
    
	transform = glm::scale(transform, glm::vec3(f_esc, f_esc, f_esc));
	transform = glm::translate(transform, -centre_esfera);  
...
{ 	// Matriu de transformació de la viewMatrix
	...
	View = glm::translate(View, glm::vec3(0,0,0)); //Euler: View Point Reference
...
{ 	// Matriu de transformació de la projectMatrix
    ...
    //Orthogonal per que el Patricio ocupi tot el viewport
    // sino al ser perspective no s'ajusta exactament a la window
    //per tal de donar sensació de profunditat
    glm::mat4 Proj (1.0f);
...
{
	glm::vec3 Vertices[4];	
	Vertices[0] = glm::vec3(-1.0, -escalat/2, -1.0);
	Vertices[1] = glm::vec3(1.0, -escalat/2, -1.0);
	Vertices[2] = glm::vec3(-1.0, -escalat/2, 1.0);
	Vertices[3] = glm::vec3(1.0, -escalat/2, 1.0);
		
	glm::vec3 Color[4];
	Color[0] = glm::vec3(1,0,0);
	Color[1] = glm::vec3(0,1,0);
	Color[2] = glm::vec3(0,0,1);
	Color[3] = glm::vec3(0,1,1);
