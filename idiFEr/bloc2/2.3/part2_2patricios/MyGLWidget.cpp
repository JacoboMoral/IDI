...
void MyGLWidget::initializeGL ()
{
    ...
    //Calcul de l'escalat
    escalat = 1;
    fesc_y = escalat/(puntmax.y-puntmin.y);  
}

void MyGLWidget::initialize_Camera ()
{
    radi_escena = 2; //Definit al .hh per tal d'ajustar la mida del terra
    radi_model = escalat/2; //Radi del nostre model Patricio
	distancia = radi_escena*2.f;
    //Inicialitzacions d'orientacio/posicio (VM)
    OBS = glm::vec3(0,0,distancia);
    VRP = glm::vec3(0,radi_model,0); //centre de la escena a 0.5 -> patricios son d'alçada 1
    up = glm::vec3(0,1,0);
    viewTransform();
    ...
    Znear = distancia-radi_escena*sqrt(2);
    Zfar = distancia+radi_escena+radi_model*sqrt(2);
...
{	// Matriu de transformació del Model
	...
	transform = glm::scale(transform, glm::vec3(fesc_y, fesc_y, fesc_y));
...
{
	glm::vec3 Vertices[4];	
	Vertices[0] = glm::vec3(-radi_escena, 0, -radi_escena);
	Vertices[1] = glm::vec3(radi_escena, 0, -radi_escena);
	Vertices[2] = glm::vec3(-radi_escena, 0, radi_escena);
	Vertices[3] = glm::vec3(radi_escena, 0, radi_escena);
...
{
	//Buffer del model Patricio1
	bufferModel();
	//Buffer del model Patricio2
	bufferModel();
...
{	//Calculem els punts maxim, minim i central de la capsa contenidora
    ...
    radi_escena = glm::distance(puntmax,centre_esfera);    
}	
