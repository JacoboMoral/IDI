...
void MyGLWidget::pos_focus()
{	//SCModel --TG-- SCAplicacio --view-- SCObservador
	glm::vec3 posFocus;  // Posicio del focus
	
	if (focus_fix)
		posFocus = glm::vec3(View * glm::vec4(focus_x, 0.5, focus_z, 1));
	else
		posFocus = glm::vec3(focus_x, 0.5, focus_z);
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 amb(0,0,0.8); //Model Ambient
  glm::vec3 diff(0,0,0.8); //Model Difus
  glm::vec3 spec(1,1,1); //Model Especular
  float shin = 100;
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(focus_x, 0, focus_z));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F: { // canviar coordenades focus SCA a SCO
	  focus_fix = !focus_fix;
	  pos_focus();
      break;
    }
    case Qt::Key_Up: { // mou el focus cap a les Z-
	  focus_z -= 0.1;
	  pos_focus();
      break;
    }
    case Qt::Key_Down: { // mou el focus cap a les Z+
	  focus_z += 0.1;
	  pos_focus();
      break;
    }
    case Qt::Key_Left: { // mou el focus cap a les X-
	  focus_x -= 0.1;
	  pos_focus();
      break;
    }
    case Qt::Key_Right: { // mou el focus cap a les X+
	  focus_x += 0.1;
	  pos_focus();
      break;
    }
