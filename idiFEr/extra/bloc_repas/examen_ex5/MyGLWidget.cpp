...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent)
{
  ...
  radiEsc = 2*sqrt(2);//sqrt(5);
  rotar = 0;
  focuscamara = ratllat = false;
  
  posFocus = glm::vec3(1, 1, 1);  // Posicio del focus
  colFocus = glm::vec3(0.8, 0.8, 0.8); //Color del focus
}
...
void MyGLWidget::initialize_Camera ()
{
    ...
    viewTransform();
    //float distancia = radiEsc*2.f;
    FOV_inicial = FOV = (float)M_PI/3.f;//2.0*asin(radiEsc/distancia);//(float)M_PI/2.f;
    ra = 1.f;
...
void MyGLWidget::bufferVaca()
{
  ...
  // Definim el material de la Vaca
  glm::vec3 amb(0.2,0.2,0.2);
  glm::vec3 diff(0.7,0.7,0.7);
  glm::vec3 spec(0.9,0.9,0.9);
  float shin = 100;
  
  int num_v = sizeof(GLfloat)*Vaca.faces().size()*3*3; 
  // Fem que aquest material afecti a tots els vèrtexs per igual
  glm::vec3 matambVaca[num_v];
  
  for (int i = 0; i < num_v; i++) {
	  matambVaca[i] = amb;
  }
  
  glm::vec3 matdiffVaca[num_v];
  
  for (int i = 0; i < num_v; i++) {
	  matdiffVaca[i] = diff;
  }
  
  glm::vec3 matspecVaca[num_v];
  
  for (int i = 0; i < num_v; i++) {
	  matspecVaca[i] = spec;
  }
  
  float matshinVaca[num_v];
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*Vaca.faces().size()*3*3, matambVaca, GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*Vaca.faces().size()*3*3, matdiffVaca, GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*Vaca.faces().size()*3*3, matspecVaca, GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*Vaca.faces().size()*3*3, matshinVaca, GL_STATIC_DRAW);
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva) {
    ...
    Proj = glm::perspective(FOV, ra, 0.05f, 10.f);
	}
  else
...
void MyGLWidget::viewTransform ()
{
  ...
  //View = glm::translate(glm::mat4(1.f), -glm::vec3(-2, 1, -2));
  ...
  View *= glm::lookAt(glm::vec3(-1, 1, -1), VRP, glm::vec3(0, 1, 0));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  esc_vaca = 0.5/(maxy-miny);
  centreVaca[0] = (minx+maxx)/2.0; centreVaca[1] = (miny+maxy)/2.0; centreVaca[2] = minz/*(minz+maxz)/2.0*/;
	minyv=miny;
...
void MyGLWidget::ratllarPantalla() {
	  glUniform1i (ratllatLoc, ratllat);
	  ratllatLoc = glGetUniformLocation (program->programId(), "ratllat");
}
...
void MyGLWidget::focusEsc() 
{
    makeCurrent();
	posFocus = glm::vec3(0, 0, 0);  // Posicio del focus
	colFocus = glm::vec3(0, 0.8, 0.8); //Color del focus	
