...
//es podria passar la matriu normal per uniform si fem:
//glm::mat3 NormalMatrix = glm::inverseTranspose(glm::mat3(View * TG));
//s'ha de tenir a View i TG com a atributs de classe i passar com a uniform.
...
void MyGLWidget::initializeGL ()
{
  ...
  glUniform3fv(llumLoc,1,&colFocus[0]);
  glm::vec3 llumambient(0.2,0.2,0.2);
  glUniform3fv(llumambLoc,1,&llumambient[0]);
}

void MyGLWidget::posFocus(){
  glm::vec3 posFocus(posllumx,1,1);
  if(!focus){
    glm::vec4 posfo(posFocus,1);
    posfo = View * posfo;
    posFocus = glm::vec3(posfo);
  }
  glUniform3fv(posLoc,1,&posFocus[0]);
  //calcular posfocu
}

void MyGLWidget::uniformNormal(){
  glm::mat3 normal = glm::inverseTranspose(glm::mat3(View * TG));
  glUniformMatrix3fv (normLoc, 1, GL_FALSE, &normal[0][0]);
}
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 amb(0,0,0.5);//model ambient(no depen normal)
  glm::vec3 diff(0,0,0.8);//model difus(no depen obs)
  glm::vec3 spec(1,1,1);//model especular(depen de tot)
  float shin = 500;
...
void MyGLWidget::carregaShaders()
{
  ...
  normLoc = glGetUniformLocation (program->programId(), "NormalMatrix");
  posLoc = glGetUniformLocation (program->programId(), "posFocus");
  llumLoc = glGetUniformLocation (program->programId(), "colFocus");
